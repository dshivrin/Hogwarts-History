...
from .models import CheckResult, WorkValidation
...
class _ValidationContext:
    ...
    def artifact_path(self, record: dict[str, Any], field: str) -> Path:
        value = record.get(field)
        if not isinstance(value, str) or not value:
            raise ValueError(f"manifest field {field} is missing")
        relative = Path(value)
        if relative.is_absolute() or ".." in relative.parts:
            raise ValueError(f"manifest field {field} is not a safe relative path")
        if relative.parts[: len(DATASET_DIRECTORY.parts)] == DATASET_DIRECTORY.parts:
            candidate = self.dataset_root.parents[1] / relative
        else:
...
def validate_work(
    ...
    *,
    ...
    metadata, metadata_error = _load_or_error(
        lambda: _load_json_object(capture_dir / "metadata.json"),
        empty={},
    )
    records, manifest_error = _load_or_error(
        lambda: _load_manifest_records(
...
def _check_chapter_count(context: _ValidationContext) -> tuple[bool, str]:
    source = _mapping(context.metadata.get("source"))
    discovery = _mapping(context.metadata.get("discovery"))
    pages = _list(context.metadata.get("pages"))
    chapters = _list(discovery.get("chapters"))
    expected = source.get("expected_available_chapter_count")
    artifact_counts = {
    ...
    observed = {
        "discovery": len(chapters),
        "captured": len(pages),
        "manifest": len(context.manifest_records),
        **artifact_counts,
    }
    passed = (
        "metadata" not in context.input_errors
        and isinstance(expected, int)
        and expected >= 1
        and all(count == expected for count in observed.values())
    )
    detail = f"displayed={expected!r}; observed={observed}"
    if "metadata" in context.input_errors:
        detail = f"{context.input_errors['metadata']}; {detail}"
...
def _check_chapter_sequence(context: _ValidationContext) -> tuple[bool, str]:
    ...
    pages = _list(context.metadata.get("pages"))
    chapter_groups = [
        ("discovery", _list(discovery.get("chapters"))),
        ("captured", [_mapping(page).get("chapter") for page in pages]),
    ]
    failures: list[str] = []
    for label, values in chapter_groups:
        chapters = [_mapping(value) for value in values]
        indexes = [chapter.get("chapter_index") for chapter in chapters]
        urls = [str(chapter.get("chapter_url", "")) for chapter in chapters]
        ...
        for index, url in zip(indexes, urls, strict=True):
            if not isinstance(index, int) or not _url_has_chapter_index(url, index):
                failures.append(f"{label} URL does not encode chapter {index!r}")
    for page in pages:
        record = _mapping(page)
        chapter_url = str(_mapping(record.get("chapter")).get("chapter_url", ""))
        if str(record.get("final_url", "")) != chapter_url:
            failures.append("captured final URL differs from chapter URL")
    detail = (
        "; ".join(failures)
        if failures
        else "chapter indexes and URLs are unique, canonical, and consecutive"
    )
...
def _check_chapter_titles(context: _ValidationContext) -> tuple[bool, str]:
    ...
    groups = [
        ("discovery", _list(discovery.get("chapters"))),
        ("captured", [
            _mapping(page).get("chapter")
            for page in _list(context.metadata.get("pages"))
        ]),
    ]
    failures: list[str] = []
    for label, chapters in groups:
        for position, value in enumerate(chapters, start=1):
            chapter = _mapping(value)
            title = chapter.get("chapter_title")
            missing = chapter.get("title_missing") is True
...
def _check_story_text(context: _ValidationContext) -> tuple[bool, str]:
    ...
    for record in context.manifest_records:
        index = record.get("chapter_index")
...
def _check_access_denial(context: _ValidationContext) -> tuple[bool, str]:
    ...
    for record in context.manifest_records:
        for field in ("raw_html_path", "clean_html_path"):
            ...
            if matches:
                findings.append(
                    f"chapter {record.get('chapter_index')} {field}: {matches}"
                )
    detail = (
        "; ".join(findings)
        if findings
...
def _check_site_chrome(context: _ValidationContext) -> tuple[bool, str]:
    ...
    for record in context.manifest_records:
        ...
        if selectors or text_matches:
            findings.append(
                f"chapter {record.get('chapter_index')} selectors={selectors} text={text_matches}"
            )
    detail = (
        "; ".join(findings)
        if findings
...
def _check_ratios(
    context: _ValidationContext,
    *,
    denominator: Callable[[dict[str, Any]], str],
    label: str,
) -> tuple[bool, str]:
    ratios: list[str] = []
    failed = False
    for record in context.manifest_records:
        ...
        ratios.append(f"chapter {record.get('chapter_index')}={ratio:.3f}")
...
def _check_pdf_bookmarks(context: _ValidationContext) -> tuple[bool, str]:
    ...
    expected = len(_list(_mapping(context.metadata.get("discovery")).get("chapters")))
    return (
...
def _check_artifact_hashes(context: _ValidationContext) -> tuple[bool, str]:
    ...
    for record in context.manifest_records:
        for path_field, hash_field in HASH_FIELDS.items():
            ...
            if stored != current:
                failures.append(
                    f"chapter {record.get('chapter_index')} {hash_field} mismatch"
                )
            else:
                verified += 1
    if not context.manifest_records:
...
def _check_annotation_hashes(context: _ValidationContext) -> tuple[bool, str]:
    ...
    expected_keys = {
        ...
        for record in context.manifest_records
        if isinstance(record.get("chapter_index"), int)
    }
    ...
    for record in context.manifest_records:
        index = record.get("chapter_index")
        key = f"chapter-{index:03d}" if isinstance(index, int) else ""
...
def _load_manifest_records(
    ...
    *,
    ...
    for line in path.read_text("utf-8").splitlines():
        ...
        if value.get("source_id") == source_id and value.get("capture_id") == capture_id:
            records.append(value)
    return sorted(records, key=lambda record: int(record.get("chapter_index", 0)))
